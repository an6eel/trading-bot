...
from models.stock_model import StockModelCreate, TrainingType
...
async def create_symbol(db: DataBase, symbol: SymbolCreate) -> SymbolDB:
    ...
    for train_type in TrainingType:
        historic_data = get_historic_data(db_symbol.symbol_name, train_type)
        await create_model(db.stocks_collection, StockModelCreate(
            symbol=db_symbol.symbol_name,
            data=historic_data,
            path="data/{}-{}-model.h5".format(db_symbol.symbol_name, train_type.value)
        ), train_type.value)
